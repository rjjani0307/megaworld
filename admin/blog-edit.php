            ...
            $targetFilePath1 = $targetDir . $extention1;
             if(in_array($fileType1, $allowTypes) AND move_uploaded_file($_FILES["file1"]["tmp_name"], $targetFilePath1) ){
             	$img1 = mysqli_real_escape_string($conn, $extention1);
		       	...
		       	$img1 = "noimg.jpg";
		         $statusMsg1 = "Blog Image Upload failed! </br>"; 
		       }
		        
           // Insert image file name into database 
          $query = "UPDATE `blog` SET `name`='$name',`image`= '$img1',`content`= '$content',`meta_desc`= '$meta_desc',`meta_key`= '$meta_key' WHERE id = '$id'";
            ...
            if($insert){ 
                ...
                $statusMsg = "Sorry, there was an error Editing Blog. </br>"; 
            } 

       	 }else{
       	     // Insert image file name into database 
          $query = "UPDATE `blog` SET `name`='$name',`content`= '$content',`meta_desc`= '$meta_desc',`meta_key`= '$meta_key' WHERE id = '$id'";
 			$insert= mysqli_query($conn,$query);
							...
							<form method="POST" enctype="multipart/form-data">
								<div class="card m-0">
									<div class="card-header">
										<div class="card-title">Edit Blog Details
									</div>
									<div class="card-body">
										<p style="color: red">
										<?php
										...
										<div class="row gutters">
											<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
												<div class="form-group">
													...
													<input type="text" class="form-control" id="Name" placeholder="Enter Blog Title" name="name" required value="<?php echo $row['name']; ?>">
												</div>
											...
											<img src="blog/<?php echo $row['image']; ?>" class="" >
										        </div>
												</div>
												</div>
												<div class="form-group">
													...
													<textarea class="form-control" name="content" rows="7" placeholder="Enter Blog Content"><?php echo $row['content']; ?></textarea>
													
												</div>
													<div class="form-group">
													<label for="url" class="col-form-label">Blog Meta Description
													</label>
													<textarea class="form-control" name="meta_desc" rows="7" placeholder="Enter Blog Content"><?php echo $row['content']; ?></textarea>
												</div>
												<div class="form-group">
													<label for="url" class="col-form-label">Blog Meta Key Words
													</label>
													<textarea class="form-control" name="meta_key" rows="7" placeholder="Enter Blog Content"><?php echo $row['content']; ?></textarea>
												</div>
												<!-- Row start -->
										<div class="row gutters">
