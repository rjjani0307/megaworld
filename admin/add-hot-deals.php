	  ...
	  $link = mysqli_real_escape_string($conn, $_POST['link']); 
            ...
            if(in_array($fileType1, $allowTypes) AND move_uploaded_file($_FILES["file1"]["tmp_name"], $targetFilePath1) ){
             	$img1 = mysqli_real_escape_string($conn, $extention1);
		       	...
		       	$img1 = "noimg.jpg";
		       		 $statusMsg1 = " Hot Deals Image Upload failed! </br>"; 
		       }

           // Insert image file name into database 
        $query = "INSERT INTO `hot_deals`(`name`, `image`, `url`,  `status`) VALUES ('$name', '$img1', '$link', '0')";
 			$insert= mysqli_query($conn,$query);

            if($insert){ 
            	unset($_POST);
                ?> <script>alert('Hot Deals Added Successfully');  
              window.location = "hot-deals.php";  </script>
                 <?php 
            }else{ 
                $statusMsg = "Sorry, there was an error uploading hot deals. </br>"; 
            } 

       	 }else{
      	$statusMsg .= "Upload Product Hot Deals Image. </br>";
      } 
	   }
   }
							...
							<form method="POST" enctype="multipart/form-data">
								<div class="card m-0">
									<div class="card-header">
										<div class="card-title">Add Hot Deals
									</div>
									<div class="card-body">
										<p style="color: red">
										<?php
										...
										<div class="row gutters">
											<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
												<div class="form-group">
													<label for="Name" class="col-form-label">Hot Deals Title
													</label>
													<input type="text" class="form-control" id="Name" placeholder="Hot Deals Title" name="name" value="<?php if (isset($_POST['name'])) {
										            	echo $_POST['name'];	} ?>">
												</div>
												<div class="form-group">
										...
										<div class="row gutters">
											<div class="col-xl-12"><br>
												<button type="submit" id="submit" name="hot_deals" class="btn btn-primary float-right">Submit</button>
											</div>
										</div>
										<!-- Row end -->
										</div>
