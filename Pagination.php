...
class Pagination{
	...
	var $fullTagOpen	= ' <div class="list-all-page"><div class="page-number">';
	var $fullTagClose	= '</div></div>';
	var $firstTagOpen	= '';
	var $firstTagClose	= '&nbsp;';
	var $lastTagOpen	= '&nbsp;';
	var $lastTagClose	= '';
	var $curTagOpen		= '<a  class="active">';
	var $curTagClose	= '</a>';
	var $nextTagOpen	= '&nbsp;';
	var $nextTagClose	= '&nbsp;';
	var $prevTagOpen	= '&nbsp;';
	...
	function createLinks(){ 
		...
		if ($numPages == 1){
			if ($this->showCount){
				$info = '<div class="all-page"><span class="page-title">Showing : ' . $this->totalRows . '</span></div>';
				return $info;
			}else{
				return '';
			}
	...
	function getAJAXlink( $count, $text) {
        if( $this->contentDiv == '')
            return '<a href="'. $this->anchorClass . ' ' . $this->baseURL . $count . '">'. $text .'';
