...
{  
  ...
  $pan = $_POST["pan"];
  $add1 = $_POST["add1"];
  $gst_photo = $_POST["gst_photo"];
  $pan_photo = $_POST["pan_photo"];
  ...
  $c_pwd = $_POST["c_pwd"];
    ...
    $user_id = strtoupper($username . rand(10000,99999));
     ...
     if($pwd == $pwd) {
            ...
            $file = $_FILES['gst_photo']['name'];
      	 	$fileName = basename($_FILES['gst_photo']['name']);
      	 	$fileType =  pathinfo($fileName, PATHINFO_EXTENSION); 
            $extention = $user_id.rand(1000, 9999).".".$fileType;
            $targetFilePath = $targetDir . $extention;
              $img = mysqli_real_escape_string($conn, $extention);
              if(in_array($fileType, $allowTypes) AND move_uploaded_file($_FILES["gst_photo"]["tmp_name"], $targetFilePath)){
              		$statusMsg ="";
		       }
		       
            $file2 = $_FILES['pan_photo']['name'];
      	 	$fileName2 = basename($_FILES['pan_photo']['name']);
      	 	$fileType2 =  pathinfo($fileName2, PATHINFO_EXTENSION); 
            $extention2 = $user_id.rand(1000, 9999).".".$fileType2;
            $targetFilePath2 = $targetDir . $extention2;
              $img2 = mysqli_real_escape_string($conn, $extention2);
              if(in_array($fileType2, $allowTypes) AND move_uploaded_file($_FILES["pan_photo"]["tmp_name"], $targetFilePath2)){
              		$statusMsg2 ="";
		       }
         
             $file3 = $_FILES['identity']['name'];
      	 	$fileName3 = basename($_FILES['identity']['name']);
      	 	$fileType3 =  pathinfo($fileName3, PATHINFO_EXTENSION); 
            $extention3 = $user_id.rand(1000, 9999).".".$fileType3;
            $targetFilePath3 = $targetDir . $extention3;
              $img3 = mysqli_real_escape_string($conn, $extention3);
              if(in_array($fileType, $allowTypes) AND move_uploaded_file($_FILES["identity"]["tmp_name"], $targetFilePath3)){
              		$statusMsg3 ="";
		       }

         $query = " INSERT INTO `users`(`user_id`, `name`, `org`, `gst`, `gst_photo`, `pan`, `pan_photo`, `add1`, `identity`, `city`, `pin`, `state`, `country`, `c_number`, `mobile`, `email`, `c_name`, `fullname`, `address`, `o_number`, `o_email`, `pwd`) VALUES ('$user_id', '$name', '$org', '$gst', '$img', '$pan', '$img2', '$add1', '$img3', '$city', '$pin', '$state', '$country', '$c_number', '$mobile', '$email', '$c_name', '$fullname', '$address', '$o_number', '$o_email', '$pwd')";
                    ...
                    <div class="col">
                        <div class="map-area">
                           <div class="map-title">
                                ...
                                <h1><a href="b2cregister.php" style="font-size: small;">Are you B2C ? Register Here</a></h1>
                            </div>
                                        ...
                                        <form method="POST" action="" id="myForm"  enctype="multipart/form-data">
                                            ...
                                            <label>GST Number   <span class="canceled">*</span></label>
                                                 <input type="text" name="gst" placeholder="GST Number" required="">
                                                  <label>GST Certificate   <span class="canceled">*</span></label>
                                                 <input type="file" name="gst_photo" placeholder="GST Certificate" required="">
                                            <label>PAN Number  <span class="canceled">*</span></label>
                                                 <input type="text" name="pan" placeholder="PAN Number" required="">
                                                   <label>PAN Card Photo  <span class="canceled">*</span></label>
                                                 <input type="file" name="pan_photo" placeholder="PAN  Card Photo" required="">
                                            ...
                                            <label>Email Id <span class="canceled">*</span> </label>
                                                 <input type="email" name="o_email" placeholder="Email Id" required="">
                                                   <label>Owner identity soft copy</label>
